...
using unitn_crypto_fintech::SHA256ValueGreaterEqGadget;
...
class SHA256ValueGreaterEqTest : public ::testing::Test {
protected:
	...
	shared_ptr<SHA256ValueGreaterEqGadget<DefaultField>> sha256_value_equal_gadget;

	SHA256ValueGreaterEqTest() {
		...
		sha256_value_equal_gadget = make_shared<SHA256ValueGreaterEqGadget<DefaultField>>(
...
TEST_F(SHA256ValueGreaterEqTest, GreaterValuesSatisfyCircuit) {
	pb->val(value1) = 2;
	pb->val(value2) = 1;
...
TEST_F(SHA256ValueGreaterEqTest, EqualValuesSatisfyCircuit) {
	pb->val(value1) = 1;
	pb->val(value2) = 1;
...
TEST_F(SHA256ValueGreaterEqTest, LessValuesDoNotSatisfyCircuit) {
	pb->val(value1) = 1;
	pb->val(value2) = 2;
...
TEST_F(SHA256ValueGreaterEqTest, CircuitGeneratesCorrectProof) {
	pb->val(value1) = 2;
	pb->val(value2) = 1;
...
TEST_F(SHA256ValueGreaterEqTest, CircuitRecognizesWrongProof) {
	pb->val(value1) = 2;
	pb->val(value2) = 1;
