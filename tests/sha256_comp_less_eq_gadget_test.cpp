...
using unitn_crypto_fintech::SHA256CompLessEqGadget;
...
class SHA256CompLessEqTest : public ::testing::Test {
protected:
	...
	shared_ptr<SHA256CompLessEqGadget<DefaultField>> sha256_comp_less_eq_gadget;

	SHA256CompLessEqTest() {
		...
		sha256_comp_less_eq_gadget = make_shared<SHA256CompLessEqGadget<DefaultField>>(
				*pb, value, commitment, "sha256_comp_less_eq_gadget");

		// Constaints enforcement
		sha256_comp_less_eq_gadget->generate_r1cs_constraints();
	}
};

TEST_F(SHA256CompLessEqTest, NegativeValueSatisfiesCircuit) {
	pb->val(value) = -1;

	sha256_comp_less_eq_gadget->generate_r1cs_witness(
			generator.generate_random_bit_vector(256));

	EXPECT_TRUE(pb->is_satisfied());
}

TEST_F(SHA256CompLessEqTest, PositiveValueDoesNotSatisfyCircuit) {
	pb->val(value) = 1;
...
TEST_F(SHA256CompLessEqTest, ZeroSatisfiesCircuit) {
	pb->val(value) = 0;
...
TEST_F(SHA256CompLessEqTest, CircuitGeneratesCorrectProof) {
	pb->val(value) = -1;
...
TEST_F(SHA256CompLessEqTest, CircuitRecognizesWrongProof) {
	pb->val(value) = -1;
