...
using unitn_crypto_fintech::SHA256CompLessGadget;
...
class SHA256CompLessTest : public ::testing::Test {
protected:
	...
	shared_ptr<SHA256CompLessGadget<DefaultField>> sha256_comp_less_gadget;
	...
	SHA256CompLessTest() {
		...
		sha256_comp_less_gadget = make_shared<SHA256CompLessGadget<DefaultField>>(
				*pb, value, commitment, "sha256_comp_less_gadget");

		// Constaints enforcement
		sha256_comp_less_gadget->generate_r1cs_constraints();
	}
};

TEST_F(SHA256CompLessTest, NegativeValueSatisfiesCircuit) {
	pb->val(value) = -1;

	sha256_comp_less_gadget->generate_r1cs_witness(
			generator.generate_random_bit_vector(256));

	EXPECT_TRUE(pb->is_satisfied());
}

TEST_F(SHA256CompLessTest, PositiveValueDoesNotSatisfyCircuit) {
	pb->val(value) = 1;
...
TEST_F(SHA256CompLessTest, ZeroDoesNotSatisfyCircuit) {
	pb->val(value) = 0;
...
TEST_F(SHA256CompLessTest, CircuitGeneratesCorrectProof) {
	pb->val(value) = -1;
...
TEST_F(SHA256CompLessTest, CircuitRecognizesWrongProof) {
	pb->val(value) = -1;
